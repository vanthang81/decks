...
// Năng lực Quản trị hệ thống KHÔNG bao giờ được có = CHỈ 'super.admin' (đỉnh quyền lực, super-only).
// CFO 15/09: Quản trị hệ thống ĐƯỢC review TOÀN CÔNG TY (mọi OKR + mọi việc) & edit MỌI OKR — nên
// 'task.viewall' + 'user.view360' KHÔNG còn bị chặn cứng nữa mà là NĂNG LỰC CẤU HÌNH ĐƯỢC ở ma trận
// Phân quyền (mặc định BẬT cho system_admin qua DEFAULT_GROUPS, admin có thể tắt nếu muốn siết riêng tư).
export const SYSADMIN_DENY_CAPS = ['super.admin'] as const;
...
export const CAPABILITIES: Capability[] = [
  ...
  { key: 'user.view360', cat: 'system', label: 'Xem hồ sơ 360° người dùng', desc: 'Xem ĐẦY ĐỦ hồ sơ 1 người: định danh, số liệu, chi tiết OKR/dự án/công việc/check-in/cuộc họp, lịch sử đăng nhập & hoạt động. Không có quyền này chỉ xem định danh + số lượng. Cấu hình được cho từng nhóm.', suggest: ['system_admin', 'okr_admin'] },
...
//  - system_admin: TẤT CẢ TRỪ 'super.admin' — review toàn công ty + edit mọi OKR (CFO 15/09);
//    các cap review riêng tư (task.viewall/user.view360) mặc định BẬT nhưng cấu hình được ở Phân quyền.
...
export function defaultGroupForRole(role: string): GroupKey {
  if (role === 'exec' || role === 'chairman' || role === 'ceo' || role === 'cfo') return 'system_admin';
