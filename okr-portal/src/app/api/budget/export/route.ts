...
// Xuất TEMPLATE ngân sách ra CSV (ai có năng lực "Quản lý Ngân sách") — sửa trong Excel rồi import lại.
export async function GET(req: NextRequest) {
  const user = await requireUser();
  if (!canManageBudget(user, await loadAccess())) return NextResponse.json({ error: 'forbidden' }, { status: 403 });
