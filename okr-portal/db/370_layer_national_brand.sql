...
BEGIN
  SELECT id, period_id, unit_id, owner_email INTO parent_id, p_period, p_unit, p_owner
    FROM okr_objectives WHERE code = 'CTY-O11' LIMIT 1;
  ...
  -- 1) Khách hàng
  IF NOT EXISTS (SELECT 1 FROM okr_objectives WHERE code = 'CTY-O12') THEN
    INSERT INTO okr_objectives (code, period_id, parent_id, level, unit_id, owner_email, title, description, status)
    VALUES ('CTY-O12', p_period, parent_id, 'company', p_unit, p_owner,
    ...
    VALUES (oid, 'CTY-O12.KR1', 'Số lượng khách hàng luỹ kế', 'number', 'increase', 'khách', 0, 1500000, 0);
  END IF;

  -- 2) Cửa hàng
  IF NOT EXISTS (SELECT 1 FROM okr_objectives WHERE code = 'CTY-O13') THEN
    INSERT INTO okr_objectives (code, period_id, parent_id, level, unit_id, owner_email, title, description, status)
    VALUES ('CTY-O13', p_period, parent_id, 'company', p_unit, p_owner,
      'Mở rộng mạng lưới cửa hàng',
      'Tăng số điểm bán (Total Cumulative). FM: 2026 = 80 cửa hàng → 2030 = 261.',
      'active')
    RETURNING id INTO oid;
    INSERT INTO okr_key_results (objective_id, code, title, metric_type, direction, unit_label, start_value, target_value, current_value)
    VALUES (oid, 'CTY-O13.KR1', 'Số cửa hàng luỹ kế', 'number', 'increase', 'cửa hàng', 0, 80, 0);
  END IF;

  -- 3) Doanh thu
  IF NOT EXISTS (SELECT 1 FROM okr_objectives WHERE code = 'CTY-O14') THEN
    INSERT INTO okr_objectives (code, period_id, parent_id, level, unit_id, owner_email, title, description, status)
    VALUES ('CTY-O14', p_period, parent_id, 'company', p_unit, p_owner,
      'Tăng trưởng doanh thu',
      'Doanh thu thuần theo FM: 2025 ~27.891 tỷ → 2026 ~73.841 tỷ → 2030 ~180.346 tỷ.',
      'active')
    RETURNING id INTO oid;
    INSERT INTO okr_key_results (objective_id, code, title, metric_type, direction, unit_label, start_value, target_value, current_value)
    VALUES (oid, 'CTY-O14.KR1', 'Doanh thu thuần cả năm', 'currency', 'increase', 'tỷ', 0, 73841, 0);
  END IF;
END $$;
